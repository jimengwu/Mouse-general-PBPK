#hill equation to describe the endocytes process
#------------------------------pbpk with sigmod described kinetic process-------------
...
F_W_Stomach: 0.006  : stott et al 1983 Stott WT, Dryzga MD, Ramsey JC. 1983. Blood-flow distribution in the mouse. J Appl Toxicol 3(6):310-312.
...
F_W_Blood : 0.049  : Unitless, Fractional blood (Brown 1997)
...
QC : 13.98 * 60 / 1000  : L/h Stott WT, Dryzga MD, Ramsey JC. 1983. Blood-flow distribution in the mouse. J Appl Toxicol 3(6):310-312.
F_Q_Brain : 0.033  : Unitless, Fraction blood flow to brain of the cardiac output (Brown 1997)
F_Q_Liver : 0.161  : Unitless, Fraction blood flow to liver of the cardiac output (Brown 1997)
F_Q_Kidney : 0.091  : Unitless, Fraction blood flow to kidney (Brown 1997)
F_Q_Spleen : 0.011  : Unitless, Fraction blood flow to spleen
...
F_BV_Liver : 0.31  : Unitless, Fraction blood volume in liver, percentage of liver table 30
...
double K_up_Liver  = (K_max_Liver  * pow(TIME,n_Liver))  / (pow(K_50_Liver, n_Liver)    + pow(TIME,n_Liver));
double K_up_Spleen = (K_max_Spleen * pow(TIME,n_Spleen)) / ( pow(K_50_Spleen, n_Spleen) + pow(TIME,n_Spleen));
double K_up_Kidney = (K_max_Kidney * pow(TIME,n_Kidney)) / ( pow(K_50_Kidney, n_Kidney) + pow(TIME,n_Kidney));
double K_up_Lung   = (K_max_Lung   * pow(TIME,n_Lung))   / ( pow(K_50_Lung, n_Lung)     + pow(TIME,n_Lung));
//double K_up_GI   = (K_max_GI     * pow(TIME,n_GI))     / ( pow(K_50_GI, n_Lung)       + pow(TIME,n_Lung));
...
M_Liver_t M_Liver_pc M_Urine M_bile AUC_Lt AUC_Kt AUC_St AUC_Lut
...
// -------------------------------------6.2 Spleen pc ------
double R_Spleen_pc = K_up_Spleen * M_Spleen_t - K_release_Spleen * M_Spleen_pc;
...
dxdt_AUC_St = C_Spleen_tissue;
// -----------------------------------7. Liver--------------------
// -----------------------------------7.1 Liver  blood --------------------
double R_bile = Kbile * (M_Liver_b / W_Liver_b);
dxdt_M_bile = R_bile;
...
double R_Liver_pc = K_up_Liver * M_Liver_t - K_release_Liver * M_Liver_pc;
...
//double C_GI = (M_GI_b + M_GI_t + M_GI_pc) / W_GI;
...
double M_Balance = (MBA + MBV + M_Lung_b + M_Lung_t + M_Lung_pc + M_Brain_b + 
                    ...
                    M_Liver_b + M_Liver_t + M_Liver_pc + M_bile + M_Urine
);

$TABLE
capture Liver  = C_Liver*1000;
...
F_W_Liver : 0.055  : (Unitless, Fractional liver tissue (Brown 1997) table 4, https://journals.sagepub.com/doi/epdf/10.1177/074823379701300401)
F_W_Brain : 0.017  : Unitless, Fractional brain tissue (Brown 1997)
F_W_Lung : 0.007  : Unitless, Fractional brain tissue (Brown 1997)
F_W_Kidney : 0.017  : Unitless, Fractional kidney tissue (Brown 1997)
F_W_Spleen : 0.005  : Unitless, Fractional spleen tissue (Brown 1997)
...
DLC_Brain : 0.000001  : due to blood brain barrier unitless
DLC_Kidney : 0.001 :unitless
DLC_Spleen : 0.001 :unitless, 100nm
DLC_Lung : 0.001:unitless
//DLC_GI : 0.001:unitless
DLC_Rest : 0.000001:unitless
// -------------------5. Diffusion limitation coefficient constants (Permeability coefficient)------------------------
...
// -------------------6. Endocytosis-related parameters------------------------
K_release_Liver : 0.0075   : h-1, 100nm
K_max_Liver     :   4      : h-1, 100nm
K_50_Liver      :   24     : h, 100nm
n_Liver         :   0.1    : Unitless, 100nm
...
K_release_Spleen : 0.003  : h-1, 100nm
K_max_Spleen     : 10     : h-1, 100nm
K_50_Spleen      : 24     :h, 100nm
n_Spleen         : 0.1    :Unitless, 100nm


K_release_Kidney : 0.01   : h-1, 100nm
K_max_Kidney     : 0.1    : h-1, 100nm
K_50_Kidney      : 24     :h
n_Kidney         : 0.1    :Unitless, 100nm

K_release_Lung   : 0.005 : h-1, 100nm
K_max_Lung       : 0.1   : h-1, 100nm
K_50_Lung        : 24    :h
n_Lung           : 0.1   :Unitless, 100nm
...
Kbile  : 0.0012   : Biliary clearance (L/hr), 100nm
Kurine : 0.00012  : Urine clearance (L/hr), 100nm
...
// -------------------------------------2.3 in the lung pc compartment----------------------------
double R_lung_pc = K_up_Lung * M_Lung_b - K_release_Lung * M_Lung_pc;
dxdt_M_Lung_pc = R_lung_pc;
double C_Lung = (M_Lung_b + M_Lung_t + M_Lung_pc) / W_Lung;
double C_Lung_tissue = (M_Lung_t + M_Lung_pc) / W_Lung_t;
dxdt_AUC_Lut = C_Lung_tissue;
...
// ------------------------------------5.2 Kidney pc------------
double R_Kidney_pc = K_up_Kidney * M_Kidney_b - K_release_Kidney * M_Kidney_pc;
dxdt_M_Kidney_pc = R_Kidney_pc;
double C_Kidney = (M_Kidney_b + M_Kidney_t + M_Kidney_pc) / W_Kidney;
double C_Kidney_tissue = (M_Kidney_t + M_Kidney_pc) / W_Kidney_t;
dxdt_AUC_Kt = C_Kidney_tissue;
...
//----------------3. Endocytosis parameter-------------
//double K_up_Liver = K_max_Liver;
double K_up_Spleen = K_max_Spleen;
//double K_up_Liver  = (K_max_Liver  * pow(TIME,n_Liver))  / (pow(K_50_Liver, n_Liver)    + pow(TIME,n_Liver));
//double K_up_Spleen = (K_max_Spleen * pow(TIME,n_Spleen)) / ( pow(K_50_Spleen, n_Spleen) + pow(TIME,n_Spleen));

double K_up_Kidney = K_max_Kidney;
double K_up_Lung   = K_max_Lung; 
//double K_up_GI   = (K_max_GI     * pow(TIME,n_GI))     / ( pow(K_50_GI, n_Lung)       + pow(TIME,n_Lung));
double K_up_GI   = K_max_GI;
...
// --------------------------- 1.1 Arterial Blood part ---------------------------------------
double K_up_Liver  = K_max_Liver * (1 - M_Liver_pc/(A_cap_liver  * W_Liver_t));  

//double K_up_Spleen = K_max_Spleen * (1 - M_Spleen_pc/(A_cap_spleen  * W_Spleen_t));  
//double K_up_Kidney = K_max_Kidney * (1 - M_Kidney_pc/(A_cap_kidney  * W_Kidney_t));  
double RBA = QC * (M_Lung_b / W_Lung_b) - QC * (MBA / (W_Plasma * 0.2));  // rate of change in Arterial blood
dxdt_MBA = RBA;
// --------------------------- 1.1 Arterial Blood part ---------------------------------------
